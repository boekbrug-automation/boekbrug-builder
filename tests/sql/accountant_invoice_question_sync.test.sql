-- migrations: invoice_accountant_status_vocabulary.sql, invoice_accountant_attribution.sql, verwerkt_freeze_level.sql, accountant_subject_status.sql, invoice_questions.sql, accountant_invoice_status_sync.sql
-- =====================================================================
...
BEGIN
  ...
  TRUNCATE public.bank_tx_invoices, public.bank_transactions, public.invoices, public.accountant_subject_status, public.accountant_clients;
...
SELECT '[VRAAG-SYNC] held: asking writes both facts; the client can read and never close; resolving closes the accountant''s own question in the same transaction; other invoices and other accountants are untouched; a failed second write rolls the first one back' AS result;
