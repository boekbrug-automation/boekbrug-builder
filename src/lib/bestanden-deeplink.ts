...
export function targetFromIntake(data: unknown): FileTarget | null {
  ...
  // [ONTVANGEN-WAAR] The receive-first shape, in camelCase where every other branch is snake_case.
  //
  // [UPLOAD-TRUTH-1] This read used to be described as "for symmetry, not because the route sends
  // it", on the belief that `focus` finds the file regardless of folder. It does not: Bestanden
  // loads ONE folder's contents and returns silently when the focused id is not among them, and a
  // received file lives in "Geïmporteerde bestanden", never the root. So the field is now actually
  // sent, and a receive-first link opens the folder the file is in. A null here still means the
  // root, which is the honest reading of an answer that named no folder — it is no longer the
  // normal case.
