...
export async function gateStorage(params: {
  ...
  try {
    ...
    if (typeof usage.storageMb !== "number") return allow; // not measurable → never a violation
  ...
  // 0 means "no ceiling" here for the same reason it does in fair_use_consume: measure before you
  // limit. A paid plan that has not chosen a number counts and never refuses.
  if (limitMb <= 0) return allow;

  // Rounded UP, so a file that lands exactly on the boundary is refused rather than admitted: the
  // meter the owner reads is whole megabytes, and a refusal must never be a rounding away from the
  // number on his screen.
  const wantMb = Math.ceil(Math.max(0, params.bytes) / (1024 * 1024));
  if (usedMb + wantMb <= limitMb) return allow;
