...
WITH intent(sig, kind, status, definer, i_anon, i_authenticated, i_service_role, i_public, dev_anon, dev_authenticated, dev_service_role, dev_public, acl_files) AS (VALUES
  ...
  ('public.is_my_accountant_client(uuid)', 'rls_helper', 'live', true, 'ALLOW', 'ALLOW', 'ALLOW', 'DENY', NULL, NULL, NULL, 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; the PUBLIC entry is not the capability the TO public policies need (the named grants are); revoking it is a separate hardening step bound by IS_MY_ACCOUNTANT_CLIENT_HARDENING_INVARIANT', ARRAY[]::text[]),
  ...
  ('public.accountant_status_door_only()', 'trigger', 'live', true, 'DENY', 'DENY', 'DENY', 'DENY', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', ARRAY[]::text[]),
  ('public.grant_welcome_plus()', 'obsolete', 'live', true, 'DENY', 'DENY', 'DENY', 'DENY', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step', ARRAY[]::text[]),
...
findings AS (
  ...
  SELECT 'ANON_EXECUTES_DEFINER_AGAINST_INTENT', l.sig,
    ...
    JOIN (VALUES
      ...
      ('public.accountant_status_door_only()', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step'),
      ('public.grant_welcome_plus()', 'resolved by the 2026-09-20 evidence pass (batch 2); production privilege deliberately unchanged in that PR; a trigger function needs no EXECUTE for any role to fire, and cannot be called directly; this grant is inert default exposure, left in place until a separate hardening step'),
...
WITH repo(name) AS (VALUES
  ...
  ('accountant_directory_publication_follows_evidence'),
